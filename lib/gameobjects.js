...
World.prototype.update_state = function(t, dt) {
  ...
  for (var player_id in players) {
    ...
    if (!player.dead) {
      ...
      // Forward BRAKE action (stop/reverse thrust) from player to entity
      entity.set(BRAKE, player.is(BRAKE));
...
Ship.prototype.move = function(t, dt) {
  ...
  var max_speed = 800;        // ↑ faster
  var acc_speed = 1000;       // ↑ stronger acceleration

  var acc = 0;
  if (this.is(THRUST)) {
    acc = dt * acc_speed;
  } else if (this.is(BRAKE)) {
    acc = -dt * acc_speed * 2;  // stronger brake
  }

  var friction = this.is(BRAKE) ? 0.90 : 0.99;

  var speedx = (this.vel[0] + acc * Math.sin(angle)) * friction;
  var speedy = (this.vel[1] - acc * Math.cos(angle)) * friction;
  ...
  this.pos = [
    this.pos[0] + speedx * dt,
    this.pos[1] + speedy * dt
  ];
};
...
Ship.prototype.isBrake = function() {
  return (this.action & BRAKE) == BRAKE;
}
